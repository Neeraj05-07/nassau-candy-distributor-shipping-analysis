...
def load_data():
    ...
    df = pd.read_csv('C:/Users/Neeraj_Thakur/OneDrive/Documents/Nassau Candy Distributor/data/nassau_candy_distributor_cleaned.csv', parse_dates=['Order Date', 'Ship Date'])
    return df
data = load_data()
...
def load_metrics():
    ...
    route_metrics = pd.read_csv('C:/Users/Neeraj_Thakur/OneDrive/Documents/Nassau Candy Distributor/matrics/route_metrics.csv')
    regional_metrics = pd.read_csv('C:/Users/Neeraj_Thakur/OneDrive/Documents/Nassau Candy Distributor/matrics/regional_metrics.csv')
    state_metrics = pd.read_csv('C:/Users/Neeraj_Thakur/OneDrive/Documents/Nassau Candy Distributor/matrics/state_metrics.csv')
    mode_metrics = pd.read_csv('C:/Users/Neeraj_Thakur/OneDrive/Documents/Nassau Candy Distributor/matrics/mode_metrics.csv')
    factory_metrics = pd.read_csv('C:/Users/Neeraj_Thakur/OneDrive/Documents/Nassau Candy Distributor/matrics/factory_metrics.csv')

    with open('C:/Users/Neeraj_Thakur/OneDrive/Documents/Nassau Candy Distributor/summary_stats.txt', 'r') as f:
...
with tab2:
    ...
    with col1:
        ...
        regional_perf.columns = ['Region', 'Avg Lead Time', 'Orders', 'Sales']
        regional_perf = regional_perf.sort_values('Avg Lead Time')
        ...
        fig_regional = px.bar(
            ...
            color='Avg Lead Time',
            color_continuous_scale='RdYlGn_r',
            hover_data={'Orders': True, 'Sales': ':.0f', 'Avg Lead Time': ':.2f'},
            title="Average Lead Time by Region"
        )
        
        fig_regional.update_layout(hovermode='x unified')
        st.plotly_chart(fig_regional, use_container_width=True)
    
    # State Performance
    with col2:
        st.subheader("State-Level Performance (Top 15)")
        
        state_perf = filtered_df.groupby('State/Province').agg({
            'Lead Time': 'mean',
            'Order ID': 'count'
        }).reset_index()
        state_perf.columns = ['State', 'Avg Lead Time', 'Orders']
        state_perf = state_perf[state_perf['Orders'] >= 2].sort_values('Avg Lead Time', ascending=False).head(15)
        ...
        fig_state = px.bar(
            ...
            color='Avg Lead Time',
            color_continuous_scale='RdYlGn_r',
            hover_data={'Orders': True, 'Avg Lead Time': ':.2f'},
            title="15 Slowest States by Lead Time"
        )
        
        fig_state.update_layout(yaxis={'categoryorder': 'total ascending'}, hovermode='y unified')
    ...
    with col1:
        # Regional Bottleneck (Volume × Lead Time)
        ...
        # Normalize
        bottleneck['LT_norm'] = (bottleneck['Avg Lead Time'] - bottleneck['Avg Lead Time'].min()) / \
                               (bottleneck['Avg Lead Time'].max() - bottleneck['Avg Lead Time'].min())
        bottleneck['Vol_norm'] = (bottleneck['Volume'] - bottleneck['Volume'].min()) / \
                                (bottleneck['Volume'].max() - bottleneck['Volume'].min())
        bottleneck['Bottleneck Score'] = (bottleneck['LT_norm'] * 0.6 + bottleneck['Vol_norm'] * 0.4) * 100
        bottleneck = bottleneck.sort_values('Bottleneck Score', ascending=False)
...
with tab5:
    ...
    time_series = filtered_df.groupby(filtered_df['Order Date'].dt.to_period('W'))['Lead Time'].agg(['mean', 'count']).reset_index()
    ...
    fig_trend = px.line(
        ...
        labels={'Avg Lead Time': 'Average Lead Time (days)'},
